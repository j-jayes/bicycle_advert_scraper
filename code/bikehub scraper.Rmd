...
Webscraping the data on bicycles from bikehub classifieds. 


### Start with getting a list of the ads

```{r}

# url for where the pages of ads are listed for all bikes
...
write.csv(list_of_links_clean, file = paste("data/bikehub_links_",st, ".csv", sep = ""))
...
html <- read_html("https://bikehub.co.za/classifieds/item/hardtail-mountain-bikes/422342/merida-big-nine-xt-editon-2018")
...
test <- get_ad_text("https://bikehub.co.za/classifieds/item/vintage-bikes/419967/colnago-titanio-10sp-chorus")
...
## Process the seller details into the same format as the gumtree one
...
st <- format(Sys.time(), "%Y-%m-%d-%I-%M-%p")
write_rds(ads, paste0("data/bike_hub_", st, ".rds"))
